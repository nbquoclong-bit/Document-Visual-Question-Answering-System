...
> **Cấu trúc:** 12 Slide Chuẩn Học Thuật & Doanh Nghiệp  
> **Cam kết tính xác thực:** 100% số liệu trên mỗi slide đều có **tên tệp tin dẫn chứng thực tế (Evidence Files)** đối soát trong mã nguồn dự án.
...
│ Slide 7 : Kết Quả Thực Nghiệm Định Lượng (Base ➔ Sau Fine-Tune ➔ Sau Optimize)         │
│ Slide 8 : Phân Tích Hiệu Năng Chi Tiết Theo Từng Nhóm Trường Kế Toán                   │
...
* **Điểm nổi bật:** Đạt **94.94% ANLS**, **92.80% F1**, **74.14% Exact Match**, xuất Full JSON 1024 Tokens với độ trễ chỉ 2.59 giây trên GPU Tesla T4.
* 📁 **File dẫn chứng số liệu:** [`model/output/optimized_evaluation_report.json`](file:///d:/STUDY/MLIoT/project/model/output/optimized_evaluation_report.json)

### 🎙️ Lời thoại thuyết trình (Speaker Script):
> *"Kính thưa Thầy và Hội đồng đánh giá, hôm nay nhóm chúng em xin phép được báo cáo đồ án với đề tài: **'Hệ thống Document Visual Question Answering & Bóc Tách Hóa Đơn Tự Động Tiếng Việt'** sử dụng mô hình Vision-Language thế hệ mới `Qwen2.5-VL-3B`. Toàn bộ số liệu và kết quả trong bài báo cáo hôm nay đều được trích xuất trực tiếp từ các tệp kiểm định thực nghiệm thực tế trong mã nguồn dự án. Trong bài thuyết trình này, nhóm sẽ trình bày toàn bộ quy trình từ chuẩn bị dữ liệu 114 nghìn mẫu, phương pháp toán học tối ưu siêu tham số, tiến trình tối ưu hóa giúp mô hình đạt 94.94% ANLS, và đặc biệt là Case Study phân tích chuyên sâu về các thử nghiệm thất bại trong bài toán Bounding Box để rút ra những bài học kỹ thuật quý giá."*
...
* 📁 **File dẫn chứng phân tích nghiệp vụ:** [`docs/TONG_HOP_MUC_TIEU_SAN_PHAM_VA_DU_LIEU.md`](file:///d:/STUDY/MLIoT/project/docs/TONG_HOP_MUC_TIEU_SAN_PHAM_VA_DU_LIEU.md#phần-1-bối-cảnh-bài-toán--mục-tiêu-sản-phẩm)
...
* 📁 **File dẫn chứng đặc tả trường kế toán:** [`docs/TONG_HOP_MUC_TIEU_SAN_PHAM_VA_DU_LIEU.md`](file:///d:/STUDY/MLIoT/project/docs/TONG_HOP_MUC_TIEU_SAN_PHAM_VA_DU_LIEU.md#phần-2-tính-năng-cốt-lõi-minh-chứng-trực-quan--bóc-tách-thông-tin-hóa-đơn)
...
* 📁 **Files dẫn chứng dữ liệu:**
  * Tập Train Master (97,508 mẫu, ~34 MB): [`model/data/vlm_train_master.json`](file:///d:/STUDY/MLIoT/project/model/data/vlm_train_master.json)
  * Tập Val Master (17,208 mẫu, ~6 MB): [`model/data/vlm_val_master.json`](file:///d:/STUDY/MLIoT/project/model/data/vlm_val_master.json)
  * Báo cáo phân tích Dataset: [`docs/BAO_CAO_CHI_TIET_DATASET.md`](file:///d:/STUDY/MLIoT/project/docs/BAO_CAO_CHI_TIET_DATASET.md)
...
### 🎙️ Lời thoại thuyết trình (Speaker Script):
> *"Về mặt dữ liệu, nhóm đã xây dựng một tập ngữ liệu đồ sộ gồm 114,716 mẫu hỏi đáp đa nhiệm được lưu trữ trực tiếp tại `vlm_train_master.json` và `vlm_val_master.json`. Bộ dữ liệu bao phủ 15 mẫu hóa đơn thực tế từ hóa đơn xăng dầu Petrolimex, cước viễn thông Viettel, đến các hóa đơn bán lẻ WinMart, Highlands. Dữ liệu được phân bổ thành 8 nhóm tác vụ khác nhau, từ việc đọc các trường số liệu đơn lẻ đến các câu hỏi suy luận logic tính toán tiền thuế, đảm bảo mô hình không bị học vẹt mà thực sự hiểu cấu trúc hóa đơn Việt Nam."*
...
* 📁 **Files dẫn chứng thuật toán & tham số:**
  * Cấu hình tham số tối ưu: [`model/optimal_hyperparameters.json`](file:///d:/STUDY/MLIoT/project/model/optimal_hyperparameters.json)
  * Module AutoML: [`model/hyperparameter_tuning.py`](file:///d:/STUDY/MLIoT/project/model/hyperparameter_tuning.py)

### 🎙️ Lời thoại thuyết trình (Speaker Script):
> *"Để tối ưu hóa quá trình huấn luyện, nhóm không chọn tham số ngẫu nhiên mà áp dụng giải thuật toán học Gradient LR Finder kết hợp tối ưu Bayes qua Optuna TPE trong module `hyperparameter_tuning.py`. Bằng cách tính đạo hàm của hàm mất mát theo logarit tốc độ học, nhóm đã xác định được điểm cực tiểu hội tụ ở $1 \times 10^{-4}$. Bộ siêu tham số vàng được lưu trong tệp `optimal_hyperparameters.json` gồm LoRA Rank 16 và Alpha 32 giúp mô hình có đủ dung lượng biểu diễn để học cả ngữ nghĩa tiếng Việt lẫn cấu trúc bảng biểu."*
...
| :---: | :--- | :--- | :--- |
| **1** | **All-Linear LoRA Targeting** | Mở rộng LoRA lên **toàn bộ 7 lớp Linear** (`q, k, v, o, gate, up, down_proj`) | Nắm bắt sâu cả cơ chế chú ý lẫn biểu diễn FFN của tiếng Việt |
| **2** | **Resolution Constraining** | Giới hạn `min_pixels=256*28*28`, `max_pixels=1024*28*28` | Giảm 50% thời gian xử lý, ngăn chặn 100% nguy cơ OOM VRAM |
| **3** | **Dynamic Token Allocation** | Cấp 1024 tokens cho Full JSON, 256 tokens cho Single QA | Khắc phục triệt để lỗi bị ngắt cụt dòng khi xuất bảng kê dài |
...
* 📁 **Files dẫn chứng kiến trúc:**
  * Cấu hình 7 lớp LoRA (37.15M params): [`model/output/qwen2_5_vl_lora_adapters/adapter_config.json`](file:///d:/STUDY/MLIoT/project/model/output/qwen2_5_vl_lora_adapters/adapter_config.json)
  * Pipeline huấn luyện trên GPU: [`kaggle_automation/train_qwen2_5_vl.py`](file:///d:/STUDY/MLIoT/project/kaggle_automation/train_qwen2_5_vl.py)

### 🎙️ Lời thoại thuyết trình (Speaker Script):
> *"Trên slide là 4 cải tiến kỹ thuật đột phá được nhóm triển khai: Thứ nhất, như được định nghĩa trong `adapter_config.json`, thay vì chỉ gắn LoRA vào 2 lớp cơ bản, nhóm mở rộng sang toàn bộ 7 ma trận trọng số bao gồm cả khối MLP. Thứ hai, cơ chế Resolution Constraining giúp khống chế số lượng visual tokens không bị bùng nổ khi gặp ảnh scan 4K. Thứ ba, cơ chế cấp phát token động 1024 Tokens giúp trích xuất JSON toàn diện. Và thứ tư, việc chuẩn hóa System Prompt giúp câu trả lời của mô hình ngắn gọn, đúng trọng tâm kế toán."*

---

## 🖼️ SLIDE 7: KẾT QUẢ THỰC NGHIỆM ĐỊNH LƯỢNG (BASE ➔ FINE-TUNE ➔ OPTIMIZE)

### 📌 Nội dung hiển thị trên Slide:
* **Môi trường kiểm thử:** 174 mẫu hóa đơn Benchmark độc lập trên cùng phần cứng Kaggle GPU Nvidia Tesla T4.
...
| **Tốc độ suy luận (Latency)** | 3.763s / câu | 4.556s / câu | **2.596s / câu** | **Nhanh hơn 43% (2.59s)** ⚡ |
...
* 📁 **Files dẫn chứng báo cáo kết quả thực nghiệm:**
  * Báo cáo Base Zero-Shot: [`model/output/qwen2_5_vl_baseline_report.json`](file:///d:/STUDY/MLIoT/project/model/output/qwen2_5_vl_baseline_report.json)
  * Báo cáo Fine-Tune Standard: [`model/output/evaluation_report.json`](file:///d:/STUDY/MLIoT/project/model/output/evaluation_report.json)
  * Báo cáo Advanced Optimized: [`model/output/optimized_evaluation_report.json`](file:///d:/STUDY/MLIoT/project/model/output/optimized_evaluation_report.json)

### 🎙️ Lời thoại thuyết trình (Speaker Script):
> *"Kính thưa Thầy, đây là bảng số liệu thực nghiệm được trích xuất 100% từ 3 file báo cáo kiểm định thực tế trong thư mục `model/output/`:
...
> 2. **Ở thế hệ thứ hai (Sau khi Fine-Tune chuẩn):** Mô hình đã bắt đầu học được ngữ cảnh kế toán, điểm F1 tăng vọt lên 73.45% và ANLS đạt 59.48%. Tuy nhiên, điểm số chưa tối đa vì bị giới hạn 256 tokens khiến các bảng kê dài bị cắt cụt giữa chừng.
> 3. **Ở thế hệ thứ ba (Sau khi Tối ưu hóa toàn diện - Advanced Optimized):** Nhờ mở rộng 7 lớp Linear LoRA, cấp phát động 1024 tokens và bộ khử nhiễu System Prompt, điểm ANLS trong `optimized_evaluation_report.json` đã có bước nhảy vọt ngoạn mục lên **94.94%**, F1 đạt **92.80%**, Exact Match đạt **74.14%** và thời gian phản hồi rút ngắn chỉ còn **2.596 giây**!"*

---

## 🖼️ SLIDE 8: PHÂN TÍCH HIỆU NĂNG CHI TIẾT THEO TỪNG NHÓM TRƯỜNG KẾ TOÁN
...
| **ITEMS_LIST (Danh sách mặt hàng)** | 28 | 0.00% | 15.56% | **99.35%** | **+83.79%** 🚀 *(Nhờ Dynamic 1024T)* |
| **TIMESTAMP (Ngày giờ lập)** | 30 | 0.00% | 10.00% | **87.42%** | **+77.42%** 🚀 *(Nhờ Prompt kế toán)* |
...
* 📁 **File dẫn chứng 174 chi tiết bản ghi:** [`model/output/optimized_evaluation_report.json`](file:///d:/STUDY/MLIoT/project/model/output/optimized_evaluation_report.json)

### 🎙️ Lời thoại thuyết trình (Speaker Script):
> *"Khi kiểm định chi tiết trên từng nhóm trường kế toán, mô hình sau khi tối ưu hóa đạt độ chính xác gần như tuyệt đối ở các trường cốt lõi: Tên đơn vị bán hàng đạt 98.37%, Tổng tiền thanh toán đạt 95.58%.
> Đặc biệt, ở trường **Danh sách mặt hàng (ITEMS_LIST)**: Ở bản Fine-Tune cũ chỉ đạt 15.56% do bị cắt cụt token; khi cấp phát Dynamic 1024 Tokens, điểm số đã tăng vọt lên **99.35%**!
...
* 📁 **File dẫn chứng báo cáo kỹ thuật Bounding Box:** [`docs/BAO_CAO_VAN_DE_BOUNDING_BOX_VA_HUONG_GIAI_QUYET.md`](file:///d:/STUDY/MLIoT/project/docs/BAO_CAO_VAN_DE_BOUNDING_BOX_VA_HUONG_GIAI_QUYET.md)

### 🎙️ Lời thoại thuyết trình (Speaker Script):
> *"Thưa Thầy và Hội đồng, bên cạnh thành công của mô hình ngôn ngữ 94.94% ANLS, nhóm muốn dành slide này để mổ xẻ một Case Study thực nghiệm rất tâm đắc đã được đúc kết trong tệp `BAO_CAO_VAN_DE_BOUNDING_BOX_VA_HUONG_GIAI_QUYET.md`:
...
* 📁 **File dẫn chứng giải pháp & bài học:** [`docs/BAO_CAO_VAN_DE_BOUNDING_BOX_VA_HUONG_GIAI_QUYET.md`](file:///d:/STUDY/MLIoT/project/docs/BAO_CAO_VAN_DE_BOUNDING_BOX_VA_HUONG_GIAI_QUYET.md#4-4-bài-học-kinh-nghiệm-đắt-giá-cho-kỹ-sư-machine-learning)
...
* 📁 **Files dẫn chứng mã nguồn triển khai:**
  * Backend API: [`backend/backend-docvqa/backend/app/main.py`](file:///d:/STUDY/MLIoT/project/backend/backend-docvqa/backend/app/main.py)
  * Frontend React: [`frontend/src/App.tsx`](file:///d:/STUDY/MLIoT/project/frontend/src/App.tsx)
  * Server Live Demo GPU: [`kaggle_automation/run_live_demo.py`](file:///d:/STUDY/MLIoT/project/kaggle_automation/run_live_demo.py)
...
### 🎙️ Lời thoại thuyết trình (Speaker Script):
> *"Về mặt triển khai phần mềm, nhóm đã đóng gói hệ thống thành một giải pháp Full-Stack hoàn chỉnh gồm giao diện React tại `frontend/src/App.tsx`, Backend FastAPI bất đồng bộ tại `backend/app/main.py` và GPU Engine chạy trên đám mây tại `run_live_demo.py`. Ngay sau đây, nhóm xin phép được mở giao diện Live Demo để Thầy và Hội đồng cùng trải nghiệm trực tiếp khả năng trích xuất thông tin của mô hình sau tối ưu trên các mẫu hóa đơn thực tế."*
...
* **Tổng kết thành tựu đề tài:**
  * ✅ Xây dựng thành công bộ dữ liệu 114,716 mẫu VQA hóa đơn tiếng Việt (`vlm_train_master.json`).
  * ✅ Tiến trình tối ưu hóa đưa mô hình đạt đỉnh cao **94.94% ANLS**, **92.80% F1** và **74.14% Exact Match** (`optimized_evaluation_report.json`).
  * ✅ Triển khai thành công ứng dụng Full-Stack bóc tách JSON và hỏi đáp thực tế dưới 2.59 giây.
  * ✅ Mổ xẻ tường tận nguyên nhân kỹ thuật và giải pháp cho bài toán Bounding Box (`BAO_CAO_VAN_DE_BOUNDING_BOX_VA_HUONG_GIAI_QUYET.md`).

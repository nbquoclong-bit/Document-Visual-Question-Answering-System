...
class SchemaAdapter:
    ...
    # Bảng ánh xạ các trường bất biến sang Canonical
    CANONICAL_KEYS = {
    ...
    def parse_vlm_response(cls, raw_input: Any) -> Dict[str, Any]:
        ...
        # Fallback: Trích xuất dạng key-value thủ công nếu mô hình trả về text tự do
        result = {}
        lines = text.split("\n")
        for line in lines:
            if ":" in line:
    ...
    def to_canonical(cls, raw_data: Any) -> Dict[str, Any]:
        ...
        # Chuẩn hóa items nếu có
        if "items" in canonical and isinstance(canonical["items"], str):
            # Nếu items là một chuỗi text, phân rã thành list
    ...
    def adapt(cls, raw_data: Any, target_format: str = "canonical") -> Dict[str, Any]:
        ...
        target_format: 'canonical' | 'misa' | 'sap' | 'fast'
        """

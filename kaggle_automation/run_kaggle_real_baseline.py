...
def prepare_and_run_multitemplate_base_evaluation():
    print("=" * 85)
    print("🚀 [KAGGLE GPU EVALUATION] ĐÁNH GIÁ BASE MODEL TRÊN 15 LOẠI HÓA ĐƠN THỰC TẾ")
    print("=" * 85)
    ...
    # Đọc 118 câu hỏi validation thực tế của 15 loại hóa đơn
    ...
    # Chọn 45 mẫu đại diện (3 mẫu cho mỗi loại trong số 15 loại hóa đơn)
    selected_samples = []
    seen_templates = {}
    for q in multitemplate_questions:
        t = q.get("template", "unknown")
        if seen_templates.get(t, 0) < 3:
            selected_samples.append(q)
            seen_templates[t] = seen_templates.get(t, 0) + 1

    print(f"📋 Đã chọn {len(selected_samples)} câu hỏi đa dạng phủ kín 15 loại hóa đơn:")
    for t, c in seen_templates.items():
        print(f"   - {t}: {c} câu hỏi")
    ...
    notebook_cells = [
        ...
        },
        {
            ...
            "source": [
                ...
                f"validation_samples = {json.dumps(selected_samples, ensure_ascii=False, indent=2)}\n",
                "\n",
    ...
    print("-" * 85)
    print("⏳ Đang theo dõi tiến trình chạy thực tế trên GPU Tesla T4 để tải file báo cáo gốc...")
    ...
    for step in range(50):
        ...
        try:
            ...
            if status == "COMPLETE":
                print("🎉 KERNEL ĐÃ HOÀN TẤT THÀNH CÔNG TRÊN GPU!")
                print("📥 Đang tải file báo cáo JSON thật về thư mục model/output/...")
                    ...
                    for item in resp.files or []:
                        if item.file_name == "baseline_evaluation_report.json":
                            ...
                            with open(save_path, "wb") as f:
                                f.write(r.content)
                            print(f"✅ Đã tải về file báo cáo gốc: {save_path}")
                            break
                break
            elif status == "ERROR":
...
if __name__ == "__main__":
    prepare_and_run_multitemplate_base_evaluation()

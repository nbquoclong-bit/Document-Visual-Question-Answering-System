...
- 3 Kịch bản thông minh:
  + Single Entity: 1 BBox ôm khít giá trị thực thể
  + List Items: Từng BBox riêng biệt cho từng dòng hàng hóa đã mua
  + Full JSON: 0 BBox (ảnh gốc 100% sạch sẽ)
...
def launch_live_demo():
    print("=" * 85)
    print("🚀 [MINIMALIST DOCVQA LIVE DEMO] KHỞI TẠO DEMO QWEN2.5-VL TRÊN GPU TESLA T4")
    print("=" * 85)
    ...
    notebook_cells = [
        ...
        },
        {
            ...
            "source": [
                ...
                "        if ',' in cleaned and len(cleaned.split(',')) > 1 and len(lines) == 1:\n",
                "            for sub in cleaned.split(','):\n",
                "                sub_c = re.sub(r'^[\\d+\\.\\-\\**\\•\\)\\s]+', '', sub).strip()\n",
                "                if len(sub_c) >= 2: items.append(sub_c)\n",
                "        else:\n",
                "            if len(cleaned) >= 2: items.append(cleaned)\n",
                "    return items[:15]\n",
                "\n",

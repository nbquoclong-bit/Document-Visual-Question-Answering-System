"""
===================================================================================
🚀 KAGGLE AUTOMATION: HUẤN LUYỆN QWEN2.5-VL-3B V2 (DOCVQA + NATIVE GROUNDING)
===================================================================================
Pipeline huấn luyện toàn diện phiên bản V2:
- Tối ưu hóa ANLS >= 95%, F1 >= 93%, EM >= 75%
    ...
    notebook_cells = [
        {
            ...
            "source": [
                ...
                "### 🎯 Mục tiêu: Đạt ANLS >= 95%, F1 >= 93% và Tự động sinh Bounding Box [ymin, xmin, ymax, xmax] trực tiếp."
            ]
        },
        {
            "cell_type": "code",
            ...
            "source": [
                ...
                "import os, sys, time, json, re, random, zipfile, torch, numpy as np\n",
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "# 2. Chuẩn Bị Bộ Dữ Liệu Multi-Task Đỉnh Cao (Text QA + Native Grounding)\n",
                ...
                "                real_p = image_map.get(bname) or image_map.get(os.path.splitext(bname)[0])\n",
                ...
                "        break\n",
                "\n",
                "print(f\"✅ Đã tổng hợp {len(train_records)} mẫu huấn luyện đa nhiệm (Text Direct + Visual Grounding).\")\n"
            ]
        },
        {
            "cell_type": "code",
            ...
            "outputs": [],
            "source": [
                "# 3. Khởi Tạo Qwen2.5-VL-3B & Cấu Hình LoRA Adapter Chuyên Biệt\n",
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "# 4. Vòng Lặp Huấn Luyện Tối Ưu (Cosine LR Scheduler & Target-Only Loss Masking)\n",
                ...
                "grad_accum_steps = 4\n",
                "\n",
                "# Tuyển chọn tập mẫu đa dạng chất lượng cao\n",
                "random.shuffle(train_records)\n",
                "train_subset = train_records[:1200]\n",
                "\n",
                "total_steps = (len(train_subset) // (batch_size * grad_accum_steps)) * epochs\n",
                "lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=total_steps, eta_min=1e-6)\n",
                "\n",
                "print(f\"🏋️ Bắt đầu huấn luyện {epochs} Epochs ({total_steps} Optimizer Steps)...\")\n",
                "model.train()\n",
                "\n",
                "loss_history = []\n",
                "global_step = 0\n",
